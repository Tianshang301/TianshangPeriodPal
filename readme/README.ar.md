...
### القيمة الحالية
> ⚠️ **تشفير قاعدة البيانات**: تم التفكير في SQLCipher لكن لم يتم تنفيذه لضمان أقصى استقرار عبر الأجهزة. يتم تخزين بياناتك في التخزين المحمي للتطبيق (غير قادر على الوصول من تطبيقات أخرى)، لكن ملف قاعدة البيانات نفسه غير مشفر. هذا يعني أنه إذا تمكن مهاجم من الوصول الفيزيائي إلى جهازك بصلاحيات root أو أدوات التحقيق الجنائي، يمكنه قراءة الملف الخام. نوصي بتعيين قفل شاشة للجهاز وتفعيل قفل التطبيق الداخلي.

### كيف يتم حماية بياناتك
- ✅ **يعمل بالكامل دون اتصال** — صفر طلبات شبكة، صفر أذونات
- ✅ **عزل صندوق رمل Android** — البيانات معزولة عن التطبيقات الأخرى ما لم يتم عمل root للجهاز
- ✅ **التخزين المحلي فقط** — لا شيء يخرج من جهازك
- ✅ **قفل التطبيق** — بصمة/وجه + رمز PIN (تجزئة Argon2id، 6 أرقام كحد أدنى)
- ✅ **الحماية من التخمين السريع** — قفل تلقائي بعد محاولات فاشلة، فترة انتظار متزايدة
- ✅ **منع اللقطات** — FLAG_SECURE مفعّل
- ✅ **لا توجد أسرار مضمونة** — بيانات البناء عبر local.properties (.gitignore)

### أفضل ممارسات الأمان
1. تعيين قفل شاشة قوي للجهاز — خط دفاعك الأول
2. تفعيل قفل التطبيق الداخلي — يمنع الوصول غير المصرح به العرضي
3. الحفاظ على تحديث نظام التشغيل والتصحيحات الأمنية — يرمّع ثغرات root المعروفة
4. لنقل البيانات، استخدم تصدير CSV واحذف الملف فوراً بعد النقل
...
> **ملاحظة**: انسخ `local.properties.example` إلى `local.properties` وأدخل بيانات توقيعك. لا تقم أبداً بعمل commit لملف `local.properties` في التحكم بال版本.
